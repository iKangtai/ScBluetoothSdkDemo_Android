...
```java
    implementation 'com.ikangtai.buletoothsdk:ScBuletoothLib:1.4.9'
```
...
```java
        ...
        dependencies {
            implementation(name: 'scbluetoothlib-release-v1.4.9', ext: 'aar')
        }
```
...
The thermometer supports the instruction set:
```java
...
```
Forehead temperature gun supports instruction set:
```java
{GET_EWQ_FIRMWARE_VERSION, GET_EWQ_DEVICE_DATA}
```
AKY3  thermometer supports instruction set:
```java
...
```
AKY4  thermometer supports instruction set:
```java
...
Body temperature patch supports instruction set:
```java
...
```java
    -keepclasseswithmembernames class * {  # keep native methods
        native <methods>;
    }
